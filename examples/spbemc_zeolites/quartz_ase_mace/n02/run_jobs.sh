...
python_supports_ase() {
	...
	"$python_executable" -c 'import ase' >/dev/null 2>&1
}

resolve_ase_python() {
	local candidate=""
	local active_conda_python=""

	if [ -n "${SOD_ASE_PYTHON:-}" ]; then
		if [ ! -x "${SOD_ASE_PYTHON}" ]; then
			echo "[run_jobs] SOD_ASE_PYTHON points to a non-executable file: ${SOD_ASE_PYTHON}" >&2
			return 1
		fi
		if python_supports_ase "${SOD_ASE_PYTHON}"; then
			printf '%s\n' "${SOD_ASE_PYTHON}"
			return 0
		fi
		echo "[run_jobs] SOD_ASE_PYTHON does not provide a working ASE installation: ${SOD_ASE_PYTHON}" >&2
		return 1
	fi

	if [ -n "${CONDA_PREFIX:-}" ]; then
		active_conda_python="${CONDA_PREFIX}/bin/python"
		if [ -x "$active_conda_python" ] && python_supports_ase "$active_conda_python"; then
			printf '%s\n' "$active_conda_python"
			return 0
		fi
	fi

	for candidate in python3 python /home/salvador/miniforge3/bin/python; do
		if [ -x "$candidate" ]; then
			if python_supports_ase "$candidate"; then
				printf '%s\n' "$candidate"
				return 0
			fi
...
run_job_ase() {
	local vasp_file="$1"
	local cpu="$2"
	local output_file="${vasp_file%.vasp}.out.ase"

	if [ ! -f "./vasp2ase.py" ]; then
		echo "[run_jobs] Missing canonical ASE driver ./vasp2ase.py." >&2
		return 1
	fi
	if [ ! -f "./template_ase.py" ]; then
		echo "[run_jobs] Missing ASE template ./template_ase.py." >&2
		return 1
	fi

	echo "[run_jobs] Running ${vasp_file} on CPU ${cpu} with ${ASE_PYTHON_EXECUTABLE}"
	PYTHONUNBUFFERED=1 MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/mpl-sod-ase}" \
		taskset -c "$cpu" "$ASE_PYTHON_EXECUTABLE" ./vasp2ase.py "$vasp_file" ./template_ase.py > "$output_file" 2>&1
}
...
if [ "$FILER_VALUE" = "14" ]; then
	GLOBAL_GULP_LIMIT=-1
	if ! ASE_PYTHON_EXECUTABLE="$(resolve_ase_python)"; then
		echo "[run_jobs] ASE-capable Python executable not found. Set SOD_ASE_PYTHON or install ASE in python3." >&2
		exit 1
	fi
fi
...
elif [ "$FILER_VALUE" = "14" ]; then
	for vasp in c[0-9][0-9][0-9][0-9][0-9].vasp; do
		[ -e "$vasp" ] || continue
		slot=$(wait_for_slot)
		cpu=${CPU_ARRAY[$slot]}
		run_job_ase "$vasp" "$cpu" &
		SLOT_PIDS[$slot]=$!
	done
else
