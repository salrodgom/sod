...
contains
    ...
    call summarize_level(level, total_sites, temperature, total_comb, real(accept_count, dp), &
                 energies(1:accept_count), best_energy, best_subset, best_count, skipped, &
                 energies_low(1:accept_count), energies_high(1:accept_count), &
                 use_parallel, summary_unit, summary_txt_unit)
...
!$omp parallel do default(shared) private(i) reduction(+:wsum, expected_sum) if(use_parallel)
        do i = 1, ncomb
            weights(i) = exp(-beta * (energies(i) - emin))
            wsum = wsum + weights(i)
...
!$omp end parallel do
        ...
        if (wsum > 0.0_dp) then
            prob_best = exp(-beta * (best_energy - emin)) / wsum
        else
            prob_best = 0.0_dp
        end if
