...
! Gestor de espacios de trabajo para los controladores ensemble Monte Carlo.
!*******************************************************************************

! Module `workspace` manages reusable buffers for the Monte Carlo workflows.
! El módulo `workspace` gestiona búferes reutilizables para los flujos Monte Carlo.
module workspace
    use consts
...
! Prepara espacios de trabajo locales al hilo con el tamaño esperado para el nivel de paralelismo.
...
! Proporciona a quien llama punteros a los búferes en caché dimensionados por ensure_capacity.
...
! Garantiza que los búferes reutilizables sean lo bastante grandes para el nivel y el número de muestras solicitados.
...
! Garantiza que un vector real tenga al menos la capacidad solicitada.
...
! Garantiza que un vector entero tenga al menos la capacidad solicitada.
...
! Garantiza que una matriz real tenga al menos la capacidad de columnas solicitada.
...
! Recupera el espacio de trabajo asignado al hilo OpenMP actual.
    subroutine get_workspace(ws)
        ...
        if (idx > size(workspaces)) then
            write(*,'(A,I0,A,I0)') 'Error: active OpenMP threads (', idx, ') exceed the allocated workspace count (', &
                size(workspaces), ').'
            stop 2
        end if
        ws => workspaces(idx)
...
! Libera todos los búferes asignados en cada espacio de trabajo por hilo.
...
! Reinicia un espacio de trabajo liberando cualquier búfer subyacente.
...
! Libera cada puntero perteneciente a una instancia de espacio de trabajo.
